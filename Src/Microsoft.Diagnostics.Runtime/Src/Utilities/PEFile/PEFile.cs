...
namespace Microsoft.Diagnostics.Runtime.Utilities
{
  ...
  public sealed unsafe class PEFile : IDisposable
  {
    ...
    public bool GetPdbSignature(out string pdbName, out Guid pdbGuid, out int pdbAge, bool first = false)
    {
      ...
      {
        var buff = AllocBuff();
        var debugEntries = (ImageDebugDirectory*)FetchRVA(Header.DebugDirectory.VirtualAddress, Header.DebugDirectory.Size, buff);
        if (Header.DebugDirectory.Size % sizeof(ImageDebugDirectory) != 0)
          return false;

        var debugCount = Header.DebugDirectory.Size / sizeof(ImageDebugDirectory);
        for (var i = 0; i < debugCount; i++)
          if (debugEntries[i].Type == ImageDebugType.CODEVIEW)
          {
