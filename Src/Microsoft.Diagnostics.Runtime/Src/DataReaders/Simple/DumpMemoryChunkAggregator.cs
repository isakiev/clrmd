      ...
      for (var i = 1; i < chunks.Count; i++)
      {
        ...
        if (prevChunk.TargetEndAddress > curChunk.TargetStartAddress)
        {
          ...
          else
          {
            ...
            curChunk = new DumpMemoryChunk(curChunk.Size - overlap, curChunk.TargetStartAddress + overlap, new ContentPosition(curChunk.ContentPosition.Value + (long)overlap));
            chunks[i] = curChunk;
